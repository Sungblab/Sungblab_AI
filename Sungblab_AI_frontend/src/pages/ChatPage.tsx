...
import MainSidebarContent from "../components/main/MainSidebar";
import { GeneralChatSidebar } from "../components/layout/common/Sidebar";
...
const ChatPage: React.FC = () => {
  ...
  const handleExampleClick = (message: string) => {
    if (chatInterfaceRef.current) {
      chatInterfaceRef.current.setNewMessage(message);
    }
  };

  const handleModelSelect = (modelId: string) => {
    if (chatInterfaceRef.current) {
      chatInterfaceRef.current.handleModelChange(modelId);
    }
  };
      ...
      <div className={`flex min-w-0 h-full ${isSidebarOpen ? "md:space-x-3 lg:space-x-6" : "md:space-x-0"} transition-all duration-300 ease-in-out`}>
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="h-full mx-auto max-w-2xl lg:max-w-4xl xl:max-w-5xl 2xl:max-w-6xl">
            <ChatInterface chatId={chatId} ref={chatInterfaceRef} />
          </div>
        </div>
